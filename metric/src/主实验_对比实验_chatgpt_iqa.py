...
##
device = "cuda:3"
picture_path = "model_output_pic/gpt3-5_v2_0"
...
def main(arg_metric, arg_picture_path, writer):
...
if __name__ == "__main__":
    ...
    ##
    writer_file = open(f"./metric_iqa/iqa_{model_name}_each_metric.csv", "w", newline="")
    writer_1 = csv.DictWriter(writer_file, fieldnames=["model", "iqa", "score"])
    writer_1.writeheader()

    for metric in metric_list:
        main(metric, picture_path, writer_1)
        writer_file.flush()

    # writer_1.close()

    writer_2 = csv.DictWriter(open(f"./metric_iqa/iqa_{model_name}_each_model.csv", "w", newline=""), fieldnames=["model"] + metric_list)
