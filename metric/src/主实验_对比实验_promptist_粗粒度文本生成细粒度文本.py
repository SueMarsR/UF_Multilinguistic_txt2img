...
with open(base_file, "r+") as f:
    ...
    for row in data:
        ...
        for i, _ in enumerate(gpt2_ans):
            row[f"ans_{i}"] = {"prompt": pre_caption(_["generated_text"].strip()).encode("utf-8")}
        output.append(row)
        pbar.update(1)
    pbar.close()

with open(f"./data_comparison/eval_images_coarse-graied_promptist.csv", "w", newline="") as file:
